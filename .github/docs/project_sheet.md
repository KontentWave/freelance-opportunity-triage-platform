...
Executed local evidence at this update: the reviewed Phase 3 PHPUnit surface passed against MariaDB 11.4 with 41 tests and 198 assertions. The final complete MariaDB suite passed with 150 tests and 1,085 assertions. PHPStan, Pint, strict Composer validation, workflow YAML parsing, and the locked dependency audit passed; the audit found no security advisories. Local coverage was not measured because this PHP installation has neither PCOV nor Xdebug, so the committed CI coverage gates and hosted protected checks remain unclaimed by this working-tree update.
